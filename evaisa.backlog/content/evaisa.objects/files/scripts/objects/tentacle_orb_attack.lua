...
local is_feral = EntityHasTag(entity_id, "feral")

if(not is_feral and GlobalsGetValue("tentacle_"..entity_id.."_feralframe", "no") ~= "no")then
    local frame_diff = GameGetFrameNum() - tonumber(GlobalsGetValue("tentacle_"..entity_id.."_feralframe", "no"))

   

    if(frame_diff > 900 and EntityHasTag(entity_id, "feral") == false)then
        GamePrint("You hear angry gurgles.")
        ...
        EntityLoad("mods/evaisa.backlog/content/evaisa.objects/files/entities/tentacle_orb_feral/tentacle_orb.xml", x, y)
        EntityLoad("mods/evaisa.backlog/content/evaisa.objects/files/entities/tentacle_orb/tentacle_effect.xml", x, y)
...
--[[
            local debug_marker = EntityLoad("mods/evaisa.backlog/content/evaisa.objects/files/entities/misc/debug_point.xml", hit_pos.x, hit_pos.y)

            EntityAddTag(debug_marker, "tentacle_debug")]]

                if(len > max_len - 5)then
--[[
                    local debug_marker = EntityLoad("mods/evaisa.backlog/content/evaisa.objects/files/entities/misc/debug_point3.xml", hit_pos.x, hit_pos.y)
...
--[[
            local debug_marker = EntityLoad("mods/evaisa.backlog/content/evaisa.objects/files/entities/misc/debug_point2.xml", current_hitpos.x, current_hitpos.y)
]]
            EntityAddTag(debug_marker, "tentacle_debug")
            ...
            if(Random(1,4) > 1)then
                ...
                if(GlobalsGetValue(entity_id.."previous_direction", "") ~= "")then
                 --   GamePrint("Keep Going")
                   angle = tonumber(GlobalsGetValue(entity_id.."previous_direction", tostring(angle)))
                   --angle = 0 - math.atan2( current_hitpos.y - y, current_hitpos.x - x )
                else
                  --  GamePrint("Change Course")
                    angle = 0 - math.atan2( current_hitpos.y - y, current_hitpos.x - x )
        ...
        if ( Random( 1, 4 ) > 1 ) then
            ...
            if ( #targets > 0 ) then
                ...
                for i,target in ipairs(targets) do
                    ...
                    local check = EntityHasTag(target, "enemy")
                    if(is_feral)then
                        check = EntityHasTag(target, "player_unit") or EntityHasTag(target, "enemy")
                    end
...
--[[
                ...
                if ( #valid_targets > 0 ) then
                    ...
                    if ( target_id ~= 0) then
                        ...
                        --[[
                        local debug_marker = EntityLoad("mods/evaisa.backlog/content/evaisa.objects/files/entities/misc/debug_point2.xml", tx, ty)
        ...
        if(angle ~= nil and player_distance < 200)then
...
--[[
            ...
            if(is_feral)then
                shoot_projectile( EntityGetRootEntity( entity_id ), "mods/evaisa.backlog/content/evaisa.objects/files/entities/tentacle_orb_feral/tentacle_projectile.xml", x, y, vel_x, vel_y )
            else
                shoot_projectile( EntityGetRootEntity( entity_id ), "mods/evaisa.backlog/content/evaisa.objects/files/entities/tentacle_orb/tentacle_projectile.xml", x, y, vel_x, vel_y )
            end
