Vector = dofile_once("mods/evaisa.backlog/content/evaisa.lidar/files/vector.lua")
Array = dofile_once("mods/evaisa.backlog/content/evaisa.lidar/files/store.lua")
...
local function DrawLine(vector1, vector2, r, g, b, a)
	...
	GuiImage(gui, new_id(), vector1.x, vector1.y, "mods/evaisa.backlog/content/evaisa.lidar/files/pixel.png", a, dist, 1, angle)
end
...
if(players[1])then
	...
	if(scan_down)then
		...
		if(fire_2_frame == GameGetFrameNum() and not not_first_scan)then
			GamePlaySound("mods/evaisa.backlog/content/evaisa.lidar/lidar.bank", "lidar/scan_start", 0, 0)
			print("scan start")
			not_first_scan = true
			...
			if(world_state ~= nil)then
				...
				if(audio_loop_component == nil)then
					local comp = EntityAddComponent2(world_state, "AudioLoopComponent", {
						file="mods/evaisa.backlog/content/evaisa.lidar/lidar.bank",
		...
		if(was_scanning)then
			GamePlaySound("mods/evaisa.backlog/content/evaisa.lidar/lidar.bank", "lidar/scan_stop", 0, 0)
			was_scanning = false
		end
		if(world_state ~= nil)then
			local audio_loop_component = EntityGetFirstComponentIncludingDisabled(world_state, "AudioLoopComponent", "lidar_audio_loop")
	...
	local function draw_point(x, y, enemy)
		if(enemy)then
			GameCreateSpriteForXFrames("mods/evaisa.backlog/content/evaisa.lidar/files/pixel_red.png", x + 0.5, y + 0.5, true, 0, 0, 2, true)
		else
			GameCreateSpriteForXFrames("mods/evaisa.backlog/content/evaisa.lidar/files/pixel_orange.png", x + 0.5, y + 0.5, true, 0, 0, 2, true)
		end
